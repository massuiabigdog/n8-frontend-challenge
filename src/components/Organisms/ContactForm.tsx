                ...
                formItems.map((item, index) => {
                    if (item.type === "textarea") {
                        ...
                        return (
                            <div key={index} className="relative mb-4">
                                <label htmlFor={item.id} className="leading-7 text-sm text-gray-600">{item.label}</label>
                                <input maxLength={item?.maxLength} onChange={(e) => setForm({ ...form, [item.name]: e.target.value })} type={item.type} id={item.id} name={item.name} className={commonStyle} placeholder={item.placeholder} />
                            </div>
                        )
                    }
                })
